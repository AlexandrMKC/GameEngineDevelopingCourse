...
{
public:
	struct AkFileHelperDesc : public AkFileDesc
	{
		AkFileHelperDesc() {}
		AkFileHelperDesc(const AkFileHelperDesc& in_rCopy) 
			: AkFileDesc(in_rCopy)
			, pLocation(in_rCopy.pLocation) 
		{}
		CAkFileLocation* pLocation = nullptr;
	};
	...
	{
		return OpenFile(AkFileOpenData(in_pszFileName, in_eOpenMode), out_fileDesc);
	}
	...
	{	
		AkFileHelperDesc* pDesc = (AkFileHelperDesc*)(&in_fileDesc);
		return pDesc->pLocation->Write(in_fileDesc.hFile, in_pData, io_transferInfo);
	}
	...
	{
		return OpenFile(AkFileOpenData(in_pszFilename, AK_OpenModeRead), out_fileDesc);
	}

	// Wrapper for system file handle closing.
	inline void CloseFile( AkFileDesc & in_fileDesc)
	{
		AkFileHelperDesc* pDesc = (AkFileHelperDesc*)(&in_fileDesc);
		pDesc->pLocation->CloseFile(in_fileDesc.hFile);
	}
	...
	{
		AkFileHelperDesc* pDesc = (AkFileHelperDesc*)(&in_fileDesc);
		return pDesc->pLocation->ReadBlocking(in_fileDesc.hFile, in_pBuffer, in_uPosition, in_uSizeToRead, out_uSizeRead);
	}
	
	/// Set whether the I/O system should presume that the "Create Sub-Folders for Generated Files" option in the Wwise project was selected.
