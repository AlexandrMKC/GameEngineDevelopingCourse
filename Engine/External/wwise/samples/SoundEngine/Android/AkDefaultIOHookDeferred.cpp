...
#define ANDROID_DEFERRED_DEVICE_NAME		AKTEXT("Android Deferred")	// Default deferred device name.
...
AKRESULT CAkDefaultIOHookDeferred::Open(const AkFileOpenData& in_FileOpen, AkFileDesc*& out_pFileDesc)
{
	...
	AKRESULT eResult = OpenFile(in_FileOpen, *out_pFileDesc);
...
{
	return CAkFileHelpers::OutputSearchedPaths(in_FileOpen, out_searchedPath, in_pathSize);
}
...
{
	AKRESULT eResult = CAkFileHelpers::Write(in_fileDesc, io_transferInfo.pBuffer, io_transferInfo);
	io_transferInfo.pCallback( &io_transferInfo, eResult );
}
...
AkUInt32 CAkDefaultIOHookDeferred::GetBlockSize( AkFileDesc &)
{
    ...
    )
{
	...
	AK_CHAR_TO_UTF16( out_deviceDesc.szDeviceName, ANDROID_DEFERRED_DEVICE_NAME, AK_MONITOR_DEVICENAME_MAXLENGTH );
