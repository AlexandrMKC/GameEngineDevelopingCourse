...
void SoundInput::Init(AK::IAkGlobalPluginContext* in_pContext)
{
	m_pPlatformFuncsService = AK_GET_PLUGIN_SERVICE_PLATFORMFUNCS(in_pContext);
}

void SoundInput::Term()
{
	InputOff();
	m_pPlatformFuncsService = nullptr;
}
	    ...
	    )
{
    ...
    while( io_pBufferOut->uValidFrames != uMaxFrames )
    {
        ...
        {
            AkUInt16 l_transferSize = (std::min)( 
                m_RemainingSize, 
...
{
	if(m_pPlatformFuncsService->IsValidThread(&m_RefillThread))
	{
		return AK_Fail;
	}
	...
	m_pPlatformFuncsService->GetDefaultHighPriorityThreadProperties( threadProps );

	m_pPlatformFuncsService->CreateThread(
					RefillThreadFunc,  // Start address
					this,              // Parameter
...
{
	if (m_pPlatformFuncsService)
	{
		m_bStopRefillThread = true;
		if (m_pPlatformFuncsService->IsValidThread(&m_RefillThread))
		{
			AKPLATFORM::AkSignalEvent(m_eventRefill);
			m_pPlatformFuncsService->WaitForSingleThread(&m_RefillThread);
			m_pPlatformFuncsService->DestroyThread(&m_RefillThread);
		}
