...
{	
	...
	// Init drawing - prefer a 1:1 canvas vs. window size, but layout is hard-coded for consistent layout on all platforms
	bSuccess = InitDrawing(in_szErrorBuffer, in_unErrorBufferCharCount, in_windowWidth, in_windowHeight, INTEGRATIONDEMO_LAYOUT_WIDTH, INTEGRATIONDEMO_LAYOUT_HEIGHT);
	if (!bSuccess)
	{
		goto cleanup;
	}
	...
	// Create a gamepad overlay for the touch-based interface
	m_pGamepadLayout = Drawing_CreateGamepadLayout();
	if (m_pGamepadLayout)
	{
		Drawing_SetVirtualGamepadLayout(m_pGamepadLayout);
		m_pInput->AddVirtualGamepad(m_pGamepadLayout);
	}
#endif
...
bool IntegrationDemo::ResetVideo(void* in_pParam, AkOSChar* in_szErrorBuffer, unsigned int in_unErrorBufferCharCount, AkUInt32 in_windowWidth, AkUInt32 in_windowHeight)
{
	// First, uninitialize everything
#if defined(INTEGRATIONDEMO_VIRTUAL_GAMEPAD_OVERLAY)
	if (m_pGamepadLayout)
	{
		m_pInput->RemoveVirtualGamepad();
		Drawing_SetVirtualGamepadLayout(nullptr);
		Drawing_DestroyGamepadLayout(m_pGamepadLayout);
	...
	bool bSuccess = RenderInit(in_pParam, in_szErrorBuffer, in_unErrorBufferCharCount);
	if (!bSuccess)
	{
		Term();
		return bSuccess;
...
void IntegrationDemo::GetDefaultSettings()
{
	...
	m_initSettings.BGMCallback = IntegrationDemo::OnBackgroundMusicChange;
	m_initSettings.BGMCallbackCookie = this;
...
#endif
	// Command Buffer Demo sends large amounts of data when the Wwise Profiler is connected.
	// Double the default monitor queue size to accomodate this data.
	m_initSettings.uMonitorQueuePoolSize *= 2;
...
AKRESULT IntegrationDemo::OnBackgroundMusicChange(bool in_bBackgroundMusicMuted, void* in_pCookie)
{
	static_cast<IntegrationDemo*>(in_pCookie)->m_bIsBackgroundMusicMuted = in_bBackgroundMusicMuted;
	return AK_Success;
}
