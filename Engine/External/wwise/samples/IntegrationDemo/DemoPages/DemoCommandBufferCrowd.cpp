...
{
	m_szHelp = "This demo shows how to use the command buffer to send high volume of data to the sound engine. "
			   "The Command Buffer API has two primary advantages over the C++ API: "
			   "\n "
			   "  1. It minimizes both CPU usage and memory allocations for large data transfers. "
			   "\n "
			   "  2. It is fully compatible with any C11 compiler, and therefore its functions can "
		       "be bound directly to other programming languages that have FFI compatibilities, such as Rust. "
			   "\n "
			   "As you enter the page you will see a crowd of people walking towards you from the left. Each "
			   "small yellow circle represents a game object emitter with a custom position sent to the sound engine. "
			   "Using <<UG_BUTTON3>> you can remove game objects from the scene and with <<UG_BUTTON4>> you can add them back. "
...
void DemoCommandBufferCrowd::Draw()
{
	...
	DrawTextOnScreen("(Press <<UG_BUTTON3>> To Remove Game Object...)", iPosX, iPosY, DrawStyle_Text);
	iPosY -= iLineHeight;
	DrawTextOnScreen("(Press <<UG_BUTTON4>> To Add Game Object...)", iPosX, iPosY, DrawStyle_Text);
