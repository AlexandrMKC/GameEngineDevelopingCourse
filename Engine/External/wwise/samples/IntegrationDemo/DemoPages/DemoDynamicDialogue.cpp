...
const char* DemoDynamicDialogue::CAPTIONS[] =
{
	...
	"Test 19 - Callback-based Dynamic Sequence.",
	"Test 20 - Seeking in currently-playing item."
};
...
DemoDynamicDialogue::DemoDynamicDialogue( Menu& in_ParentMenu ):Page( in_ParentMenu, "Dynamic Dialogue Demo" )
{
	...
	m_pTestFunctions[0] = &DemoDynamicDialogue::Set1_1_SimpleSequenceUsingID;
	m_pTestFunctions[1] = &DemoDynamicDialogue::Set2_1_SimpleSequenceUsingString;
	m_pTestFunctions[2] = &DemoDynamicDialogue::Set3_1_StartPlayback;
	m_pTestFunctions[3] = &DemoDynamicDialogue::Set4_1_StartPlayback;
	m_pTestFunctions[4] = &DemoDynamicDialogue::Set5_1_StartEmptyPlaylist;
	m_pTestFunctions[5] = &DemoDynamicDialogue::Set6_1_StartPlayback;
	m_pTestFunctions[6] = &DemoDynamicDialogue::Set7_1_StartPlayback;
	m_pTestFunctions[7] = &DemoDynamicDialogue::Set8_1_StartPlayback;
	m_pTestFunctions[8] = &DemoDynamicDialogue::Set9_1_UsingDelay;
	m_pTestFunctions[9] = &DemoDynamicDialogue::Set10_1_StartPlayback;
	m_pTestFunctions[10] = &DemoDynamicDialogue::Set11_1_StartPlayback;
	m_pTestFunctions[11] = &DemoDynamicDialogue::Set12_1_StartPlayback;
	m_pTestFunctions[12] = &DemoDynamicDialogue::Set13_1_StartPlayback;
	m_pTestFunctions[13] = &DemoDynamicDialogue::Set14_1_StartPlaybackWithCallback;
	m_pTestFunctions[14] = &DemoDynamicDialogue::Set15_1_StartPlaybackWithCallback;
	m_pTestFunctions[15] = &DemoDynamicDialogue::Set16_1_StartPlaybackWithCallback;
	m_pTestFunctions[16] = &DemoDynamicDialogue::Set17_1_StartPlaybackWithCallback;
	m_pTestFunctions[17] = &DemoDynamicDialogue::Set18_1_StartPlayback;
	m_pTestFunctions[18] = &DemoDynamicDialogue::Set19_1_StartPlayback;
	m_pTestFunctions[19] = &DemoDynamicDialogue::Set20_1_StartPlayback;
}
...
bool DemoDynamicDialogue::Update()
{
	...
	else
	{
		...
		if ( ( !m_bPause || m_bTestInProgress ) && m_pNextFunction )
		{
			...
			// Also update the selector to keep the UI in sync
			static_cast<ToggleControl*>(m_Controls[1])->SetSelectedIndex(m_uSetIndex-1);
		}
	}
...
void DemoDynamicDialogue::InitControls()
{
	...
	// Create test selection toggle
	ToggleControl* newToggle = new ToggleControl(*this);
	newToggle->SetLabel("Starting test:");
	for (int i = 0; i < NUM_TESTS; i++)
	{
		newToggle->AddOption(m_captions[i+1].c_str(), (void*)(AkUIntPtr)i);
	}
	newToggle->SetSelectedIndex(0);
	newToggle->SetDelegate((PageMFP)&DemoDynamicDialogue::Test_Selected);
...
void DemoDynamicDialogue::PlayButton_Pressed( void* in_pSender, ControlEvent* )
{
	...
	if ( ! m_pNextFunction && ! m_bTestInProgress )
	{
		...
		m_pNextFunction = m_pTestFunctions[m_uSetIndex];
		m_bPause = false;
...
void DemoDynamicDialogue::StopAndResetTests()
{
	...
	if (mSet16_Seq1PlayingID != 0)
	{
		...
		mSet16_Seq1PlayingID = 0;
	}
	if (mSet16_Seq2PlayingID != 0)
	{
		AK::SoundEngine::DynamicSequence::Playlist* pPlaylist = AK::SoundEngine::DynamicSequence::LockPlaylist( mSet16_Seq2PlayingID );
		if( pPlaylist )
		{
...
void DemoDynamicDialogue::Test_Selected(void* in_pSender, ControlEvent* in_pControlEvent)
{
	StopAndResetTests();
	m_uSetIndex = (AkUInt8)(AkUIntPtr)((ToggleControl*)in_pSender)->SelectedValue();
}
...
void DemoDynamicDialogue::Set14_Callback( AkCallbackType in_eType, AkEventCallbackInfo* in_pEventInfo, void* in_pCallbackInfo, void* in_pCookie )
{
	DemoDynamicDialogue* pDemoInstance = (DemoDynamicDialogue*)in_pCookie;
...
void DemoDynamicDialogue::Set15_Callback( AkCallbackType in_eType, AkEventCallbackInfo* in_pEventInfo, void* in_pCallbackInfo, void* in_pCookie )
{
	DemoDynamicDialogue* pDemoInstance = (DemoDynamicDialogue*)in_pCookie;
...
void DemoDynamicDialogue::Set16_Callback( AkCallbackType in_eType, AkEventCallbackInfo* in_pEventInfo, void* in_pCallbackInfo, void* in_pCookie )
{
	DemoDynamicDialogue* pDemoInstance = (DemoDynamicDialogue*)in_pCookie;
	
	if ( in_eType == AK_EndOfEvent && in_pEventInfo->playingID == mSet16_Seq1PlayingID )
	{
...
void DemoDynamicDialogue::Set17_Callback( AkCallbackType in_eType, AkEventCallbackInfo* in_pEventInfo, void* in_pCallbackInfo, void* in_pCookie )
{
	DemoDynamicDialogue* pDemoInstance = (DemoDynamicDialogue*)in_pCookie;
...
void DemoDynamicDialogue::Set18_5_PlayRestOfSequence()
{
	...
	m_bTestInProgress = false;
	m_pNextFunction = &DemoDynamicDialogue::Set19_1_StartPlayback;
	Wait( 6500 );
}

void DemoDynamicDialogue::Set19_1_StartPlayback()
{
	// Starting Test Set 19
	m_bTestInProgress = true;
	m_pNextFunction = NULL;
	++m_uSetIndex;
	mSet19_State = 0;

	// Specifying an event callback with the AK_DynamicSequenceSelect bit set bypasses the Playlist entirely in favor of a callback system.
	// Each time the sequence must determine the next item to play, DemoDynamicDialogue::Set19_Callback will be called with callback type AK_DynamicSequenceSelect.
	m_PlayingID = AK::SoundEngine::DynamicSequence::Open(GAME_OBJECT_RADIO, AK_DynamicSequenceSelect, &DemoDynamicDialogue::Set19_Callback, this);

	// Because we are using AK_DynamicSequenceSelect, we can Play the sequence immediately without filling the Playlist.
	// On the next Sound Engine tick, DemoDynamicDialogue::Set19_Callback will be called to determine the first item to play.
	AK::SoundEngine::DynamicSequence::Play(m_PlayingID);

	// Let the state machine run.
	m_bTestInProgress = false;
	m_pNextFunction = &DemoDynamicDialogue::Set19_2_ResumePlayback;
	Wait(2500);
}

void DemoDynamicDialogue::Set19_2_ResumePlayback()
{
	AK::SoundEngine::DynamicSequence::Play(m_PlayingID);

	// All test sets finished. Wait a bit for playback to finish and reset the tests.
	m_bTestInProgress = false;
	m_pNextFunction = &DemoDynamicDialogue::Set20_1_StartPlayback;
	Wait(500);
}

void DemoDynamicDialogue::Set19_Callback(AkCallbackType in_eType, AkEventCallbackInfo* in_pEventInfo, void* out_pCallbackInfo, void* in_pCookie)
{
	if (in_eType == AK_DynamicSequenceSelect)
	{
		// Sound Engine is requesting information about the next item to play in our dynamic sequence.
		// To communicate this, the callback must fill out the information contained in out_pCallbackInfo.
		DemoDynamicDialogue* pDemo = static_cast<DemoDynamicDialogue*>(in_pCookie);
		AkDynamicSequenceSelectCallbackInfo* pDynSeqItem = static_cast<AkDynamicSequenceSelectCallbackInfo*>(out_pCallbackInfo);

		// For this demonstration, we use a simple state machine to determine the next item to play.
		switch (pDemo->mSet19_State)
		{
		case 0:
		{
			// First element: Comm out
			AkArgumentValueID walkieTalkieArgument = AK::STATES::WALKIETALKIE::STATE::COMM_IN;
			pDynSeqItem->audioNodeID = AK::SoundEngine::DynamicDialogue::ResolveDialogueEvent(AK::DIALOGUE_EVENTS::WALKIETALKIE, &walkieTalkieArgument, 1);
			break;
		}
		case 1:
		{
			// Second element: Speech with delay
			const AkOSChar* underAttackStringArguments[] = { AKTEXT("Unit_A"),
														AKTEXT("Gang"),
														AKTEXT("Hangar") };

			// Resolve the dialogue Event using the arguments strings array.
			pDynSeqItem->audioNodeID = AK::SoundEngine::DynamicDialogue::ResolveDialogueEvent(AKTEXT("Unit_Under_Attack"), underAttackStringArguments, 3);
			pDynSeqItem->msDelay = 200;
			break;
		}
		case 2:
		{
			// Pause. By omitting to fill out pDynSeqItem, the dynamic sequence will stop automatically. 
			// This callback will not be called again until the call to AK::SoundEngine::DynamicSequence::Play in Set19_2_ResumePlayback.
			break;
		}
		case 3:
		{
			// Last element: Comm out
			const AkOSChar* szWalkieTalkieOutArgument[] = { AKTEXT("Comm_Out") };
			pDynSeqItem->audioNodeID = AK::SoundEngine::DynamicDialogue::ResolveDialogueEvent(AKTEXT("WalkieTalkie"), szWalkieTalkieOutArgument, 1);
			break;
		}
		default:
		{
			// We are done
			AK::SoundEngine::DynamicSequence::Close(pDemo->m_PlayingID);
			pDemo->m_PlayingID = 0;
			break;
		}
		}
		pDemo->mSet19_State++; // Advance the state machine
	}
}

void DemoDynamicDialogue::Set20_1_StartPlayback()
{
	// Starting Test Set 20
	m_bTestInProgress = true;
	m_pNextFunction = NULL;
	++m_uSetIndex;
	...
	if (pPlaylist)
	{
		// Add three items to the playlist
		AkArgumentValueID walkieTalkieArgument = AK::STATES::WALKIETALKIE::STATE::COMM_IN;
		AkUniqueID audioNodeID = AK::SoundEngine::DynamicDialogue::ResolveDialogueEvent(AK::DIALOGUE_EVENTS::WALKIETALKIE, &walkieTalkieArgument, 1);
		pPlaylist->Enqueue(audioNodeID);
		...
		AkArgumentValueID statusArguments[] = { AK::STATES::UNIT::STATE::UNIT_A,
												AK::STATES::HOSTILE::STATE::BUM,
												AK::STATES::LOCATION::STATE::HANGAR };
		audioNodeID = AK::SoundEngine::DynamicDialogue::ResolveDialogueEvent(AK::DIALOGUE_EVENTS::UNIT_UNDER_ATTACK, statusArguments, 3);
		pPlaylist->Enqueue(audioNodeID);
	...
	m_pNextFunction = &DemoDynamicDialogue::Set20_2_SeekRelative;
	Wait(1000);
}

void DemoDynamicDialogue::Set20_2_SeekRelative()
{
	// Seek to the middle of the speech clip by specifying a relative position of 50%
	AK::SoundEngine::DynamicSequence::Seek(m_PlayingID, (AkReal32)0.5, false);

	m_pNextFunction = &DemoDynamicDialogue::Set20_2_SeekAbsolute;
	Wait(1000);
}

void DemoDynamicDialogue::Set20_2_SeekAbsolute()
{
	// Rewind to the beginning of the speech clip by specifying an absolute position of 0
	AK::SoundEngine::DynamicSequence::Seek(m_PlayingID, (AkTimeMs)0, false);
