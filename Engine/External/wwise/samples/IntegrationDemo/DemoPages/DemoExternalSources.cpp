...
bool DemoExternalSources::Init()
{
	...
	// "$(WwiseExePath)\FilePackager.Console.exe" -generate "$(WwiseProjectPath)\myFilePackagerProjects\ExternalSources (Windows).wfpproj"
...
void DemoExternalSources::InitControls()
{
	...
	// Add the "Play 4 5 6" button
	newBtn = new ButtonControl(*this);
	newBtn->SetLabel("Play \"1 2 3\" using Wave files");
	newBtn->SetDelegate((PageMFP)&DemoExternalSources::Play123Wave_Pressed);
	m_Controls.push_back(newBtn);
}
...
void DemoExternalSources::Play123_Pressed( void*, ControlEvent* )
{
	...
	sources[0].szFile = "01.wem";
	sources[0].idCodec = AKCODECID_ADPCM;
	
	sources[1].iExternalSrcCookie = AK::SoundEngine::GetIDFromString("Extern_2nd_number");		
	sources[1].szFile = "02.wem";
	sources[1].idCodec = AKCODECID_ADPCM;
		
	sources[2].iExternalSrcCookie = AK::SoundEngine::GetIDFromString("Extern_3rd_number");		
	sources[2].szFile = "03.wem";
	sources[2].idCodec = AKCODECID_ADPCM;
...
void DemoExternalSources::Play456_Pressed( void*, ControlEvent* )
{
	...
	sources[0].iExternalSrcCookie = AK::SoundEngine::GetIDFromString("Extern_1st_number");
	sources[0].szFile = "04.wem";
	sources[0].idCodec = AKCODECID_ADPCM;
	
	sources[1].iExternalSrcCookie = AK::SoundEngine::GetIDFromString("Extern_2nd_number");		
	sources[1].szFile = "05.wem";
	sources[1].idCodec = AKCODECID_ADPCM;
		
	sources[2].iExternalSrcCookie = AK::SoundEngine::GetIDFromString("Extern_3rd_number");		
	sources[2].szFile = "06_high.wem";
...
void DemoExternalSources::Play123Wave_Pressed(void*, ControlEvent*)
{
	// Set up external source data for the 3 external sources contained 
	// in each sound of the sequence container.
	// All 3 sounds are streamed (specified by file name).
	...
	// the sound's Contents Editor.
	sources[0].iExternalSrcCookie = AK::SoundEngine::GetIDFromString("Extern_1st_number");
	sources[0].szFile = "01.wav";
	sources[0].idCodec = AKCODECID_PCM_WAV;

	sources[1].iExternalSrcCookie = AK::SoundEngine::GetIDFromString("Extern_2nd_number");
	sources[1].szFile = "02.wav";
	sources[1].idCodec = AKCODECID_PCM_WAV;

	sources[2].iExternalSrcCookie = AK::SoundEngine::GetIDFromString("Extern_3rd_number");
	sources[2].szFile = "03.wav";
	sources[2].idCodec = AKCODECID_PCM_WAV;
