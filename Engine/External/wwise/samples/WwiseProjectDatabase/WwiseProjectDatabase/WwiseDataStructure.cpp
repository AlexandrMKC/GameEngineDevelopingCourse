...
WwiseDataStructure::WwiseDataStructure(const WwiseDBString& InDirectoryPath, const WwiseDBString* InPlatform)
{
    ...
    WwiseDBString CollapsedPath = InDirectoryPath.GetPath();
    CollapsedPath.CollapseRelativeDirectories();
    
    IWwiseDirectoryVisitor<WwiseDirectoryVisitor> DirectoryVisitor(new WwiseDirectoryVisitor(InPlatform));
    DirectoryVisitor.IterateDirectory(WwiseDBString(*CollapsedPath));
    auto Directory = DirectoryVisitor.Get();
    if (!Directory.IsValid())
    {
        WWISE_DB_LOG(Error, "Invalid Generated Directory %s", *CollapsedPath);
        return;
    }
    ...
    if (InPlatform)
    {
        ...
        for (const auto& Platform : ProjectInfoPlatforms)
        {
            if (Platform.Name == *InPlatform)
            {
                ...
                RequestedPlatformPath = CollapsedPath / Platform.Path;
                ...
                {
                    RequestedPlatformPath = CollapsedPath / PlatformPath;
                }
                else
                {
    ...
    {
        WWISE_DB_LOG(Verbose, "Parsing Wwise data structure at: %s...", *CollapsedPath);
    }
...
void WwiseDataStructure::LoadDataStructure(WwiseGeneratedFiles&& Directory)
{
    ...
    WwiseParallelFor(Directory.Platforms, [this, &RootFuture, &Directory](const auto& Platform)
    {
        ...
        PlatformData.GeneratedPlatformFiles = Directory.Platforms.FindRef(PlatformRef);
        ...
        if (Pair.GetSecond())
        {
            ...
            if (SharedRootFile->ProjectInfo)
            {
                ...
                for (WwiseRefIndexType LanguageIndex = 0; LanguageIndex < ProjectInfo.Languages.Size(); ++LanguageIndex)
                {
                    ...
                    if(Language.bDefault)
                    {
                      DefaultLanguage = WwiseDBSharedLanguageId(Language.Id, *Language.Name, Language.bDefault ? WwiseDBLanguageRequirement::IsDefault : WwiseDBLanguageRequirement::IsOptional);
                    }
    ...
    Triggers(WwiseRefTrigger::FGlobalIdsMap::GlobalIdsMap)
{
            ...
            if (SharedRootFile->SoundBanksInfo)
            {
                ...
                for (WwiseRefIndexType SoundBankIndex = 0; SoundBankIndex < SoundBanksInfo.SoundBanks.Size(); ++SoundBankIndex)
                {
                    ...
                    for (WwiseRefIndexType MediaIndex = 0; MediaIndex < SoundBank.Media.Size(); ++MediaIndex)
                    {
                        ...
                        OutsourcedRefMedias.AddOrReplace(mediaLanguageKey, std::move(outsourcedRefMedia));
    ...
    {
        WwiseOutsourcedRefMedia outsourcedRef = OutsourcedRefMedia.second;
...
bool WwisePlatformDataStructure::GetFromId(WwiseRefMedia& OutRef, WwiseDBShortId InShortId, WwiseDBShortId InLanguageId, WwiseDBShortId InSoundBankId) const
{
    ...
    if ((!Result))
    {
        WWISE_DB_LOG(Warning, "Could not find Media %" PRIu32 " (Lang=%" PRIu32 "; SB=%" PRIu32 "_wwise_db", InShortId, InLanguageId, InSoundBankId);
        return false;
    }
...
void WwisePlatformDataStructure::AddMediaCount(const WwiseDBArray<WwiseMetadataMedia>& InMedia)
{
    ...
    {
        const WwiseDatabaseMediaIdKey MediaId{ Media.Id, 0 };
...
void WwisePlatformDataStructure::AddSoundBankCount(const WwiseDBShortId InId, const WwiseDBShortId InLanguageId, const unsigned int InIncrement)
{
