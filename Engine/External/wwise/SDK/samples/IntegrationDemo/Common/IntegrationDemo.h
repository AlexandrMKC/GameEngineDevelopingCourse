...
public:
	...
	bool Init(
		...
		AkUInt32 in_windowWidht = INTEGRATIONDEMO_LAYOUT_WIDTH,
		AkUInt32 in_windowHeight = INTEGRATIONDEMO_LAYOUT_HEIGHT
			  );
	...
	/// Reinitializes the video component using the new window dimensions
	/// \return Whether the reset succeeded. On failure, IntegrationDemo is shut down and no longer works.
	bool ResetVideo(void* in_pParam, AkOSChar* in_szErrorBuffer, unsigned int in_unErrorBufferCharCount, AkUInt32 in_windowWidth, AkUInt32 in_windowHeight);
	...
	inline bool IsBackgroundMusicMuted() const { return m_bIsBackgroundMusicMuted; };
	...
	static AKRESULT OnBackgroundMusicChange(bool in_bBackgroundMusicMuted, void* in_pCookie);
private:
	...
	bool m_bGamePaused;
	bool m_bIsBackgroundMusicMuted;
